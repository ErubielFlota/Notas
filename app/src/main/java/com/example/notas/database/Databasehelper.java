...
public long insertNote(String note){
    ...
    public int updateNote(Note nota){
        SQLiteDatabase db=this.getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put(Note.COLUMN_NOTE,nota.getNote());

        return db.update(Note.TABLE_NAME,values, Note.COLUMN_ID+" = ?",new String[]{String.valueOf(nota.getId())});

    }
    public void deleteNote(Note nota){
        SQLiteDatabase db=this.getWritableDatabase();
        db.delete(Note.TABLE_NAME, Note.COLUMN_ID+" =?",new String[]{String.valueOf(nota.getId())});
        db.close();
